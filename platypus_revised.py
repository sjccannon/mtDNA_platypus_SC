...
def m3234GtoA_filter(vcf_location):
	#re_ID = re.compile("platypus_m\.3243A")
	#generate a list of the bam files generated by platypus - one bam file for each patient.
	list_of_files = os.listdir(vcf_location)
	for key in patient_bam_dict:
		
	#create a file to write the filtered vcf
	with open ("filtered_Mt3234_varaints.vcf", "a") as out_file:
		#slice the unwanted lines from each file so they are not repeatedly appended to the filtered_vcf
		#generate headings for the output vcf by slicing them from the first file in the listed files
		#specify the first file and open it 
		open_first = open(vcf_location + list_of_files[0], "r")
		with open_first as in_file:
			#iterate through the lines in the first file and slice out the ones needed for headers
			for line in itertools.islice(in_file, 47, 48):
				#write the headers to the filtered vcf_file and add a sample_ID column
				outfile.write("Sample_ID\t" + line)
	#iterate through all the filenames in the vcf_location
	for filename in os.listdir(vcf_location):
		#open each vcf_file in turn, ####may need to re-open the out file####
		with open(vcf_location + filename, "r") as vcf_file:
			#isolate the each patient ID to append to the filtered vcf outfile
			sample_ID = str(filename).strip('.vcf')
		#iterate through each line in each file
		for line in file:
			#identify the mitochondrial variants 
			if line.startswith("MT"):
				#split each line so the 3243 position can be individually reffered to
				columns = line.split()
				#if the integer value in column 1 is equal to 3243
				if int(columns[1]) == 3243:
					#write the line it is on to the outfile with the sample ID
